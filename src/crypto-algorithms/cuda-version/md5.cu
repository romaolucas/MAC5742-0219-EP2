...
int main() {
    ...
    if (err != cudaSuccess) {
        fprintf(stderr, "Problemas na copia do vetor");
        exit(EXIT_FAILURE);
    }

    err = cudaMalloc(&d_hash, MD5_BLOCK_SIZE * sizeof(BYTE));
    if (err != cudaSuccess) {
        fprintf(stderr, "Problemas para alocar memoria para o d_hash\n");
        exit(EXIT_FAILURE);
    }
